...
- 所有源码注释、JSDoc、Markdown 文档、ADR、开发说明和脚本注释必须使用中文。
- 代码标识符、命令 ID、配置键、文件路径、第三方工具名和协议固定字段保持原始英文。
- `@vitest-environment`、JSDoc 类型指令等工具要求的机器可读注释属于技术语法例外。
- 官方许可证原文必须保留；中文译文只能作为不具法律效力的参考文本。
- 用户可见文案继续通过国际化资源维护，不得把中文源码规范等同于取消多语言能力。
