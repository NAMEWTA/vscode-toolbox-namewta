...
- [x] `AC-001` 至 `AC-007` 均有可定位证据；AC-004、AC-005 的真实主题截图未运行，用户于 2026-08-12 明确接受该残余风险。
- [x] 验证矩阵已记录到 `<Path>{roots.state}/specdev/changes/2026-08-10-fix-toolbox-command-context-and-blame-layout/evidence/T-01.md</Path>`；人工视觉 E2E 的 `not-run` 状态未被改写为通过。
...
用户于 2026-08-12 明确要求全部 change 关闭并接受 AC-004/AC-005 真实主题截图、缩放和水平滚动人工步骤未在当前会话执行的残余风险。该决定解除本 Ticket 的阻塞，但不把未运行的人工步骤改写为 pass；后续若需要补充真实 UI 证据，应通过新 change 记录。
