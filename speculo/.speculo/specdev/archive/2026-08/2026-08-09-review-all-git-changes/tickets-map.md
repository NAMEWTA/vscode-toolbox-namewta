...
| T-04 | `<Path>{roots.state}/specdev/changes/2026-08-09-review-all-git-changes/ticket/04-verify-git-review-integration-gate.md</Path>`   | 真实 Extension Host、键盘、质量门禁和 VSIX 完整 Evidence      | T-03       | standard | medium | yes   | codex-local | AC-001 至 AC-020                                                       | Wave 4 / G-04 | done   |
              ...
              └─→ G-02: Evidence + gate closure
                    └─→ T-03 [READY, VS Code UX]
                          └─→ G-03: Evidence + gate closure
                                └─→ T-04 [DONE, integration gate; manual AC-020 waived by user]
...
- **Wave 4 / G-04 Release Candidate Gate：** T-04 自动化发布候选证据、Git 零写入审计、路径审计和状态同步完成；人工 AC-020 未运行，由用户 2026-08-12 明确豁免并接受残余风险后关闭 Gate。
