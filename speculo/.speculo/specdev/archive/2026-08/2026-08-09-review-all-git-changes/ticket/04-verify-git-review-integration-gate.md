...
- [x] AC-001 至 AC-020 均在 `<Path>{roots.state}/specdev/changes/2026-08-09-review-all-git-changes/evidence/T-04.md</Path>` 中映射到自动或手动证据；AC-020 的人工步骤按用户 2026-08-12 明确决定接受未运行状态及残余风险。
- [x] `pnpm test:integration` 与 `pnpm check:ci` 实际通过；生成的 VSIX 路径和 package list 已记录。
- [x] 键盘、辅助技术、Trust、真实 Git 特殊状态、刷新、失败恢复和 dispose 的可用证据已记录；其中键盘/辅助技术人工步骤未运行，按用户确认作为验收豁免保留。
- [x] Git 状态审计证明产品流程没有执行写入或远程操作，临时夹具已完整清理。
- [x] 实际修改已在更新后的 `writable_paths` 和 `D-T04-001`、`D-T04-002` 范围内，发现产品缺陷时均回到原 owner 路径修复。
- [x] G-04 已在 Evidence 和用户确认的验收豁免后关闭，并记录开始/关闭时的 HEAD、工作区摘要和实际修改路径；`D-R01` 的发布仍不作为人工 Gate 的通过证据。
- [x] Ticket、Tickets Map、Evidence 和 change 状态一致，所有路径偏差均已明确记录。

## 11. 用户验收豁免

用户于 2026-08-12 明确要求全部 change 关闭并接受 AC-020 人工键盘/辅助技术步骤未在当前会话执行的残余风险。该决定解除本 Ticket 的阻塞，但不把未运行的人工步骤改写为 pass；后续若需要补充真实 UI 证据，应通过新 change 记录。
